...
\item Instantiations have been added to word values and small values. This is
  ...
  pointers. To give an example, assume that $\high w$ is a word value referring
...
\begin{figure}
  {\footnotesize
    \begin{tabular}{lrcl}
      ...
      \textit{heap label assignments}  & $\Psi_{\mathrm{h}}$ & ::= & $\{\ell_{\mathrm{h}} |-> \tau, \dots, \ell_{\mathrm{h}} |-> \tau\}$ \\
...
\item Also in \cref{chap:types}, we will see that our typing judgments have in
  ...
  the same goal, in a more simple fashion, using the syntactic equality from
  Agda's standard library.
\end{itemize}

To summarize: We believe that \ATAL is fundamentally compatible with STAL
(though somewhat simpler), with the only significant missing features being
compound stacks (and hence exceptions) along with existential
types. Additionally many examples of existential type usage in STAL seems to be
re-writable using parametric polymorphism.\footnote{The author have not yet seen
  an example that could not be rewritten. He conjectures that all cases can in
  fact be rewritten, though he has not spent much time pondering it in depth.}
...
The judgment $\evalcode{\high G}{\high w}{\high I}$ is the judgment which
fetches a basic block from the global collection. In \ATALe this is very simple:
The word value must be a $\texttt{globval}$ and we use this to look up the basic
block. In \ATAL this is complicated by the fact we use this judgment to run
substitutions on the fetched basic block.
...
Finally the core of our small-step relation. We have a single judgment for each
instruction, except for \texttt{beq}, where we have a conditional jump.

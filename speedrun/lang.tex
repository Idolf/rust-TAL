...
\begin{itemize}
\item Compound stacks and existential types have been removed from \ATAL.
...
\item STAL had an unlimited number of registers, while \ATAL has a fixed number
  of registers. The exact number does not matter for any of our proofs, though
  it required that the number is fixed.
\item In STAL, heaps contained both code and data. This was split out in the
  ...
  influence on some of the other changes.
\item Tuples have been tagged with their type. STAL did not have a fully
  decidable type system, as it could not type-check program states with tuples
  on the heap. As we will see later, this change is what allows us to type-check
  any program state for \ATAL (in reasonable time). If one wishes to remain
  compatible with STAL, we could simply disallow tuples too with no negative
  effect.
...
\item In \cref{chap:types} we will introduce our subtyping relation. Here we
  will introduce the last minor change. In STAL, there were two sources of
  subtyping: A tuple could be cast to a less initialized one and a function
  could be cast to one that had more requirements for its registers. \ATAL only
  has the first source of subtyping, though it is the authors belief that any
  use if this feature could alternatively have been achieved using parametric
  polymorphism.
\item Also in \cref{chap:types}, we will see that our typing judgments have in
  ...
  implementation is equivalent and slightly simpler. In particular it has
  avoided the judgment $\Delta |- \tau = \tau$, since we believe that any
  purpose it serves can be achieved in Agda using simpler means.
\end{itemize}
...
examples of existential types usage in STAL could be rewritten using parametric
polymorphism instead.\footnote{The author have not yet seen an example that
  could not be rewritten. He conjectures that all cases can in fact be
  rewritten, though he has not spent much time considering it in depth.}

...
private
  ...
  mutual
    ...
    τ-outside-ctx {Δ} {ι = ι} (valid-α⁼ l)
      (subst-α-¬inst (subst-inst-> len+ι<ι'))
      with NP.1+n≰n
        (NP.<-trans (↓-to-< l)
                    (Nat-≤-trans (NP.m≤m+n (suc (length Δ)) ι) len+ι<ι'))
    ... | ()
    τ-outside-ctx {Δ} {ι = ι} (valid-α⁼ l)
      (subst-α-¬inst (subst-weaken-≥ len+ι<ι'))
      with NP.1+n≰n
        (Nat-≤-trans (↓-to-< l)
                     (Nat-≤-trans (NP.m≤m+n (length Δ) ι) len+ι<ι'))
    ... | ()
    τ-outside-ctx {Δ} {ι = ι} (valid-α⁼ l) (subst-α-inst sub-τ)
    ...
    τ-outside-ctx {Δ} {ι = ι} (valid-∀ Δ⋆ Γ⋆) (subst-∀ {Δ' = Δ'} sub-Δ sub-Γ)
    ...
    σ-outside-ctx {Δ} {ι = ι} (valid-ρ⁼ l)
      (subst-ρ-¬inst (subst-inst-> len+ι<ι'))
      with NP.1+n≰n
        (NP.<-trans (↓-to-< l)
                    (Nat-≤-trans (NP.m≤m+n (suc (length Δ)) ι) len+ι<ι'))
    ... | ()
    σ-outside-ctx {Δ} {ι = ι} (valid-ρ⁼ l)
      (subst-ρ-¬inst (subst-weaken-≥ len+ι<ι'))
      with NP.1+n≰n
        (Nat-≤-trans (↓-to-< l)
                     (Nat-≤-trans (NP.m≤m+n (length Δ) ι) len+ι<ι'))
    ... | ()
    σ-outside-ctx {Δ} {ι = ι} (valid-ρ⁼ l) (subst-ρ-inst sub-σ)
      with NP.1+n≰n (Nat-≤-trans (NP.m≤m+n (suc (length Δ)) ι ) (↓-to-< l))
    ... | ()
    ...
    Δ-outside-ctx [] [] = refl
    Δ-outside-ctx {Δ} {ι = ι} {v₁ = a₁ ∷ Δ₁} {v₂ = a₂ ∷ Δ₂}
                  (a⋆ ∷ Δ⋆) (sub-a ∷ sub-Δ)
      rewrite Δ-outside-ctx Δ⋆ sub-Δ
            | sym (+-assoc (length Δ₂) (length Δ) ι)
            | sym (List-length-++ Δ₂ {Δ})

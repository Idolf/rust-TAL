...
\drop{w}(\Lambda\ \Delta \cdot w[\theta_1, \dots, \theta_n]) &= \drop{w}(w) \\\\
...
\drop{g}(\mathtt{code}[\Delta]\Gamma \cdot I) = \mathtt{code}\ (\drop{I}(I)) \\\\
...
\drop{\iota}(\mathtt{add}\ r_a, r_b, v) &= \mathtt{add}\ r_a, r_b, \drop{v}(v) \\
\drop{\iota}(\mathtt{sub}\ r_a, r_b, v) &= \mathtt{sub}\ r_a, r_b, \drop{v}(v) \\
...
\drop{\iota}(\mathtt{ld}\ r_a, r_b(n)) &= \mathtt{ld}\ r_a, r_b(n) \\
\drop{\iota}(\mathtt{st}\ r_a(n), r_b) &= \mathtt{st}\ r_a(n), r_b \\
...
\drop{P}(H, R, I) = (\drop{H}(H), \drop{R}(R), \drop{I}(I)) \\\\

\high\embed : \highlang -> \simplelang \\
\high\embed(\high\running(G, P)) = \simple\running(\drop{G}(G), \drop{P}(P)) \\
\high\embed(\high\halted) = \simple\halted \\
\end{array} \\\\
